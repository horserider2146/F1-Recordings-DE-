...
BATCH_SIZE    = 16
EPOCHS        = 25
LR_HEAD       = 1e-4   # classifier + acoustic MLP
LR_BERT       = 2e-5   # unfrozen BERT layer (must be much smaller)
DROPOUT       = 0.5
PATIENCE      = 5
...
with open(os.path.join(BASE, 'models', 'scaler_unfreeze.pkl'), 'wb') as f:
...
# -- 4. Model (last BERT layer unfrozen) ----------------------------------------
class F1EmotionModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.bert = DistilBertModel.from_pretrained('distilbert-base-uncased')

        # Freeze all BERT params, then unfreeze only the last transformer layer
...
print(f'Params: {total:,} total / {trainable:,} trainable (last BERT layer unfrozen)')

# -- 5. Loss & optimiser (differential LRs) ------------------------------------
criterion = nn.CrossEntropyLoss(weight=class_weights)

bert_layer_params = list(model.bert.transformer.layer[-1].parameters())
head_params       = list(model.acoustic_mlp.parameters()) + list(model.classifier.parameters())

optimizer = torch.optim.AdamW([
    {'params': bert_layer_params, 'lr': LR_BERT},
...
best_val_f1     = 0.0
patience_count  = 0
best_model_path = os.path.join(BASE, 'models', 'best_model_unfreeze.pt')
history         = []
...
# Save history
pd.DataFrame(history).to_csv(os.path.join(BASE, 'models', 'history_unfreeze.csv'), index=False)
print('Done. history saved to models/history_unfreeze.csv')
