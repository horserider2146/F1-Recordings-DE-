...
with open(os.path.join(BASE, 'models', 'scaler_deberta.pkl'), 'wb') as f:
...
# -- 4. Model (last 2 DeBERTa encoder layers unfrozen) -------------------------
class F1EmotionModel(nn.Module):
    def __init__(self):
        ...
        # Unfreeze last 2 encoder layers for more capacity than DistilBERT run
        ...
        # DeBERTa-v3-small hidden size = 768
    ...
    def forward(self, input_ids, attention_mask, acoustic, token_type_ids=None):
        outputs = self.bert(
            input_ids=input_ids,
...
bert_params = [p for layer in model.bert.encoder.layer[-2:] for p in layer.parameters()]
...
def run_epoch(loader, train=True):
    ...
    with torch.set_grad_enabled(train):
        for batch in loader:
            ...
            logits = model(ids, mask, acou, token_type_ids=ttids)
...
best_val_f1     = 0.0
patience_count  = 0
best_model_path = os.path.join(BASE, 'models', 'best_model_deberta.pt')
history         = []
...
pd.DataFrame(history).to_csv(os.path.join(BASE, 'models', 'history_deberta.csv'), index=False)
...
print('Done. History saved to models/history_deberta.csv')
