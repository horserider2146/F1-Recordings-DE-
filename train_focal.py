...
# Mild alpha weights — softer than before (Frustrated was 5.0, now 2.0)
...
with open(os.path.join(BASE, 'models', 'scaler_focal.pkl'), 'wb') as f:
...
best_val_f1     = 0.0
patience_count  = 0
best_model_path = os.path.join(BASE, 'models', 'best_model_focal.pt')
history         = []
...
for epoch in range(1, EPOCHS + 1):
    ...
    print(f"{epoch:<4} {tr_loss:<10.4f} {tr_f1:<8.4f} {vl_loss:<10.4f} {vl_f1:<8.4f} {current_lr:<10.2e} {status}")
...
pd.DataFrame(history).to_csv(os.path.join(BASE, 'models', 'history_focal.csv'), index=False)

# -- 9. Test evaluation (inline, no separate torch.load hang risk) -------------
...
print('Done. history saved to models/history_focal.csv')
